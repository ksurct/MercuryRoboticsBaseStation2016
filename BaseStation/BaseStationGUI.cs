 ...
 *  
 * Authors: Richard Habeeb, Brandon Dunn, Daniel Marts, Daniel Wagner and Doug Anjard.
 *
 *
 * 2016 Modification authors: Dan Wagner, Matt Imel
 */
...
namespace BaseStation
{
    public partial class BaseStationGUI : Form
    {
        ...
        /// <summary>
        /// For sensor label testing purposes when not connected to robot.
        /// </summary>
        Random r = new Random();
        int updateTimer = 0;
    ...
    /*        if (ipMode.ShowDialog() != DialogResult.OK) System.Environment.Exit(0);
        ...
        private void BaseStationGUI_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (updateTimer == 3)
            {
                FrontLeftSensorReading.Text = r.Next(10, 20).ToString();
                FrontRightSensorReading.Text = r.Next(10, 20).ToString();
                RearLeftSensorReading.Text = r.Next(10, 20).ToString();
                RearRightSensorReading.Text = r.Next(10, 20).ToString();
                updateTimer = 0;
            }
            else updateTimer++;
